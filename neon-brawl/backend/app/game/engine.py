...
TICK_RATE = 30
NETWORK_RATE = int(os.environ.get("NETWORK_RATE", "30" if os.environ.get("APP_EDITION") != "internet" else "25"))
PROTOCOL_VERSION = 13
PLAYER_SPEED = 300
BULLET_SPEED = 760
CHAT_MAX_LENGTH = 120
...
class Room:
    ...
    def configure_bots(self, count, difficulty):
        ...
        for index in range(count):
            ...
            if ready:
                ...
                self.select_role(bot, random.choice(BOT_ROLES), time.monotonic())
        self.sync_pickups()
    ...
    def navigation_direction(self, start_x, start_y, goal_x, goal_y):
        ...
        def cell_center(cell):
            column, row = cell
            return min(WIDTH - PLAYER_RADIUS, column * NAV_CELL + NAV_CELL / 2), min(
                HEIGHT - PLAYER_RADIUS, row * NAV_CELL + NAV_CELL / 2)
    ...
    def update_bots(self, now):
        for bot in self.bots():
            ...
            bot["input"].update(move_x=move_x, move_y=move_y, angle=aim,
                                shoot=distance < 720 and (not low_health or random.random() < 0.45) and
                                random.random() < settings["fire_chance"],
...
async def websocket(request):
    ...
    try:
        ...
        if mode not in {"classic", "items", "pure", "profession"}:
            await ws.send_json({"type": "error", "code": "invalid_mode", "message": "玩法模式无效"})
            return ws
        name = " ".join(str(first.get("name", "玩家")).split())[:12]
        if not name:
            await ws.send_json({"type": "error", "code": "invalid_name", "message": "昵称不能为空"})
            return ws
        if code in rooms:
            room = rooms[code]
            if room.mode != mode:
        ...
        player = {"id": pid, "name": name, "x": 0, "y": 0,
        ...
        now = time.monotonic()
        await ws.send_json({"type": "welcome", "protocol": PROTOCOL_VERSION, "edition": os.environ.get("APP_EDITION", "internet"), "network_rate": NETWORK_RATE,
                    "id": pid, "player_id": pid, "room": code, "mode": mode, "width": WIDTH, "height": HEIGHT,
        ...
        async for msg in ws:
            if msg.type == WSMsgType.TEXT:
                try:
                    ...
                    await ws.send_json({"type": "error", "code": "invalid_message", "message": "消息格式无效"})
                    continue
                ...
                elif data.get("type") in {"select_role", "role"}:
                ...
                elif data.get("type") in {"bots", "configure_bots"}:
                    if os.environ.get("APP_EDITION") != "offline":
                        await ws.send_json({"type": "error", "code": "offline_only", "message": "当前后端未启用离线人机"})
                    else:
                        room.configure_bots(data.get("count", 0), str(data.get("difficulty", "normal")))
                else:
                    await ws.send_json({"type": "error", "code": "unknown_message", "message": "不支持的消息类型"})
...
async def health(_):
    return web.json_response({"game": "neon-brawl", "edition": os.environ.get("APP_EDITION", "internet"),
                              "status": "ok", "protocol": PROTOCOL_VERSION, "network_rate": NETWORK_RATE})


async def config(_):
    edition = os.environ.get("APP_EDITION", "internet")
    return web.json_response({"edition": edition, "offline_bots": edition == "offline",
                              "protocol": PROTOCOL_VERSION, "width": WIDTH, "height": HEIGHT})
...
if __name__ == "__main__":
    default_host = "127.0.0.1" if os.environ.get("APP_EDITION") == "offline" else "0.0.0.0"
    web.run_app(app, host=os.environ.get("HOST", default_host), port=int(os.environ.get("PORT", 8080)))
